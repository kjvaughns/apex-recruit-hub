...
BEGIN
  ...
  SELECT trim(both '"' from value) INTO v_setting_unlicensed FROM public.system_settings WHERE key = 'unlicensed_overview_calendly_url';
  SELECT trim(both '"' from value) INTO v_setting_fallback FROM public.system_settings WHERE key = 'licensed_fallback_calendly_url';
  SELECT coalesce(nullif(trim(both '"' from value),'')::boolean, true) INTO v_allow_recruiter FROM public.system_settings WHERE key = 'allow_recruiter_licensed_priority';
  SELECT coalesce(nullif(trim(both '"' from value),'')::boolean, true) INTO v_allow_manager FROM public.system_settings WHERE key = 'allow_manager_licensed_priority';
  ...
  IF v_allow_recruiter AND r.assigned_recruiter_id IS NOT NULL THEN
    SELECT trim(both '"' from p.licensed_calendly_url) AS licensed_calendly_url, coalesce(nullif(trim(p.full_name),''), p.email) AS n
      INTO v_prof
